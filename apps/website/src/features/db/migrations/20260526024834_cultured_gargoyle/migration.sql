ALTER TABLE `runs` ADD `updated_at` integer DEFAULT (current_timestamp) NOT NULL;--> statement-breakpoint
...
CREATE TABLE `__new_runs` (
	...
	`actions` text DEFAULT '{"is-cancelable":true,"is-confirmable":false,"is-discardable":false,"is-force-cancelable":false}' NOT NULL,
	`permissions` text DEFAULT '{"can-apply":true,"can-cancel":true,"can-comment":true,"can-discard":true,"can-force-execute":true,"can-force-cancel":true,"can-override-policy-check":true}' NOT NULL,
...
INSERT INTO `__new_runs`(`id`, `organization_id`, `workspace_id`, `configuration_version_id`, `status`, `has_changes`, `auto_apply`, `refresh`, `is_destroy`, `plan_only`, `message`, `created_at`, `position_in_queue`, `actions`, `permissions`, `variables`) SELECT `id`, `organization_id`, `workspace_id`, `configuration_version_id`, `status`, `has_changes`, `auto_apply`, `refresh`, `is_destroy`, `plan_only`, `message`, `created_at`, `position_in_queue`, `actions`, `permissions`, `variables` FROM `runs`;--> statement-breakpoint
